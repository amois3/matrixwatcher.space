...
class Scheduler:
    ...
    def register_task(
        ...
        # Randomized start phase: tasks with the same interval must not poll in
        # lockstep, otherwise their anomalies co-occur by sampling schedule rather
        # than by nature (a spurious-correlation artifact). Phase spread over the
        # full interval decorrelates this at the source.
        # Cap the start spread at 30s so the first reading is prompt (health
        # recovers quickly) while still breaking millisecond-level lockstep; the
        # per-tick jitter below handles longer-term phase drift.
        task._next_run = time.time() + random.uniform(0, min(interval, 30.0))
