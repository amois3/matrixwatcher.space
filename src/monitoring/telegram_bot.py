...
🕐 {self._format_timestamp(timestamp)}"""
 ...
 emoji = "📈"
 direction = "positive"
 meaning = "grow/fall together"
 else:
 emoji = "📉"
 direction = "negative"
 meaning = "move in antiphase"
...
💡 <b>Interpretation:</b> Parameter {meaning}"""
 ...
 timing = f"{param1} precedes {param2} on {abs(lag_seconds)} sec"
 else:
 direction = f"{param2} -> {param1}"
 timing = f"{param2} precedes {param1} on {abs(lag_seconds)} sec"
...
🎯 This may indicate on causal-consequential connection!
...
🕐 {self._format_timestamp()}"""
 ...
 text = f"""🚨 <b>CLUSTER ANOMALIES</b>

⚠️ <b>Multiple simultaneous anomalies!</b>

📊 <b>Number of:</b> {anomaly_count} anomalies
...
💡 <b>Interpretation:</b>
Simultaneous butanomalies in different systems may indicate:
• Global event
• System failure
• Hidden connection
...
🕐 {self._format_timestamp()}"""
 ...
 period_text = f"{period_seconds/3600:.1f} hour"
 else:
...
💡 <b>Interpretation:</b>
Parameter demonstrates cyclical behavior
 period ~{period_text}
