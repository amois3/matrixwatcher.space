...
class EnhancedMessageGenerator:
 ...
 if cluster.level == 1:
 msg += "Brief deviation detected in one source. "
 msg += "Such fluctuations occur regularly and stay within background noise."
 
 elif cluster.level == 2:
 msg += "Multiple independent sources showed deviations within close time window. "
 msg += "Brief process synchronization detected."
 
 elif cluster.level == 3:
 msg += "Stable cluster of deviations detected across multiple independent domains. "
 msg += "Observed behavior exceeds normal background."
 
 elif cluster.level == 4:
 msg += "Synchronized anomalies detected in physical, digital and probabilistic sources. "
 msg += "State exceeds standard operating parameters."
 
 elif cluster.level == 5:
 msg += "Rare configuration of synchronized anomalies detected across multiple domains. "
 msg += "Such events stand out in the entire observation history."
 ...
 msg += f"• Deviation from baseline: {snapshot.baseline_ratio:.1f}x\n"
 else:
 ...
 msg += f"Randomness: {randomness:.1%} (expected {expected:.1%})\n"
 msg += f"Source: {source_text}\n"
 
 # Explanation
 if randomness < 0.90:
 msg += "-> <i>Quantum numbers showing patterns</i>\n"
 msg += "-> <i>Possible randomness glitch</i>"
 elif randomness < 0.93:
 msg += "-> <i>Slight deviation from ideal randomness</i>"
 
 # Additional details
 if "autocorrelation" in meta:
 autocorr = meta["autocorrelation"]
 if abs(autocorr) > 0.1:
 msg += f"\n-> <i>Numbers correlating (r={autocorr:.2f})</i>"
 ...
 if abs(change) > 10:
 msg += f"({'rise' if change > 0 else 'drop'} on {abs(change):.1f}%)\n"
 msg += "-> <i>Sudden weather change</i>"
 else:
 msg += "\n"
 ...
 msg += f"News: {count} (usually {normal})\n"
 
 if count > normal * 2:
 msg += "-> <i>Sharp news spike</i>\n"
 msg += "-> <i>Possible major event</i>"
 ...
 msg += f"{network}: block time {block_time}\n"
 msg += "-> <i>Anomaly in speedrise blocks</i>"
 ...
 explanations.append("-> Possible influence on quantum level")
 
 if "crypto" in sources and "earthquake" in sources:
 if explanations:
 explanations.append("") # Empty line
 explanations.append("Market reaction on natural event")
 explanations.append("-> Investors reacting on news")
 ...
 if explanations:
 explanations.append("") # Empty line
 explanations.append("Solar activity may affect on other systems")
 ...
 explanations.append("-> Possible global event")
 ...
 msg += f"(avg time: {avg_time:.1f}, n={observations})\n"
 
 msg += "\n<i>-> Statistics only on based on history. Not a forecast.</i>"
 ...
 if cluster.level == 1:
 msg += "Observation no action needed"
 elif cluster.level == 2:
 msg += "Ataboveincreased attention"
 elif cluster.level >= 3:
