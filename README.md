...
The three-part status strip beneath the site's introduction updates from the UTC daily digest and live coverage, the frozen future coincidence screen, and the published future-data signal list. It distinguishes a recorded 30-second coincidence from a tested result. A missing digest or coverage report is shown as unknown, never as a quiet day. The future screen publishes no interim significance test. After its 2027-01-23 UTC endpoint, the strip points to the report for coverage and control review; even a low adjusted p-value remains exploratory and needs independent replication. Scientific claims and this README change only after that review, not automatically when an event or p-value appears.

Visitors can opt into **PWA Web Push** from the introduction. Research notifications are sent when a frozen future-study endpoint report becomes available for review, when a future-data-tested signal is newly published in the public signal panel, or when an independently replicated finding is manually published. The separate 30-second coincidence notification option is off by default; every such message explicitly calls the event an observation, not a discovery. A two-minute systemd timer polls published reports and observations, keeps per-browser delivery receipts in a private SQLite file, retries transient delivery failures and removes expired subscriptions. A browser test button checks the subscription. The private VAPID key and subscription endpoints live under `logs/push/`, outside Git; they must survive deploys and be backed up privately. Notifications require explicit browser permission and device support; delivery is best effort, so the status strip and Discovery Workbench remain the authoritative public record. On iOS/iPadOS, users must add the site to the Home Screen before opting in. Unsubscribing removes that browser's server record.

The [public findings registry](research/findings.json) starts empty. It is an **editorial gate**, never written by anomaly detectors, p-value calculations or the push dispatcher. To publish a finding, review source and pipeline coverage, the frozen analysis and all timing controls, then document an independent replication. A reviewed change to the registry must provide a stable `id`, clear `title`, `status: "independently_replicated"`, UTC `published_at` Unix seconds, and HTTPS `report_url` and `replication_url`; update this README and the site's introduction at the same time. The [registry API](https://matrixwatcher.space/api/research/findings) feeds the site's scientific-conclusion strip. Publishing an entry then sends one research push to existing opted-in browsers. A promising endpoint report alone never enters the registry.
...
Before enabling push delivery, install the Python dependencies, run `python -m src.monitoring.web_push init` once from the project directory, then install and enable `ops/systemd/matrix-watcher-push.service` and `.timer` as user units. `GET /api/push/config` should return `enabled: true`. Keep `logs/push/vapid-private.pem` (mode 0600) stable; rotating it invalidates browser subscriptions. The push dispatcher sends only after a visitor explicitly subscribes. The endpoint report alert is a request for human review, not an automatic finding.
