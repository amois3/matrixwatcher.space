...
def get_active_predictions(use_cache: bool = True) -> list[dict]:
 ...
 HONEST SYSTEM: Show only BEST prediction for each event.
 - Group by event type
 - Select freshest and most specific
 - Show its data honestly
 """
 # Check cache first
 ...
 source_count = condition.count("_") + 1 # L2_crypto_quantum = 2 source
 timestamp = p.get("timestamp", 0)
 observations = p.get("observations", 0)
 
 # Priority: specificity > freshness > reliability
 return (
 source_count, # More sourcesin = more interesting
 timestamp, # Fresher = more relevant
 min(observations, 1000) # More reliable, but not give huge numbers tomindominate
)
